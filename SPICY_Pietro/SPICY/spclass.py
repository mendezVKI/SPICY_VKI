...
#round near eps because for reason if you put 15 in python it gives you 14.99999999994
def roundDown(x): 
    xtemp=np.copy(x)
    xtemp[x==0]=1
    exponent = np.ceil(np.log10(np.abs(xtemp))) #we don't want to accidentally try and get an imaginary log (it won't work anyway)
