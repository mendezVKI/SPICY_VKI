...
data = scipy.io.loadmat('..' + os.sep + 'FluentSol.mat')
...
# Remove the points
X_p = X[inlet_and_wall_remover]
Y_p = Y[inlet_and_wall_remover]
U_p = U[inlet_and_wall_remover]
V_p = V[inlet_and_wall_remover]
P_p = P[inlet_and_wall_remover]
...
ax.scatter(X, Y, c=np.sqrt(U**2 + V**2))
ax.set_aspect(1)
fig.tight_layout()
...
poly1 = geometry.Polygon([i for i in [p1, p2, p3, p4]])  # BOX one

# Define also a second box
p1 = geometry.Point(0.0, 0)
p2 = geometry.Point(0.5, 0)
p3 = geometry.Point(0.5, 0.45)
p4 = geometry.Point(0.0, 0.45)
poly2 = geometry.Polygon([i for i in [p1, p2, p3, p4]])

# Plot the boxes with the velocity field
fig, ax = plt.subplots(figsize=(7, 4), dpi=100)
ax.quiver(X, Y, U, V)
ax.plot(*poly2.exterior.xy, 'b') # in case you want to see them for info
ax.plot(*poly1.exterior.xy, 'r') # in case you want to see them for info


# Prepare the SPICY object
SP_vel = Spicy([U_noise, V_noise], [X, Y], basis='gauss', verbose=verbose, model='laminar')
# Clustering
SP_vel.collocation([3, 6, 50, 200], Areas=[poly1, poly2, None, None], r_mM=[0.03, 0.6], eps_l=0.87, method='clustering')
...
SP_vel.plot_RBFs(level=1)
SP_vel.plot_RBFs(level=2)
SP_vel.plot_RBFs(level=3)
...
X_out = X_p[indices_test]
Y_out = Y_p[indices_test]
U_out = U_p[indices_test]
V_out = V_p[indices_test]
...
axes[0,0].scatter(X, Y, c=U_calc, s=10, vmin=0, vmax=1.3)
axes[1,0].scatter(X, Y, c=U, s=10, vmin=0, vmax=1.3)
...
_, valid_idcs = np.unique(np.column_stack((X_Pres_N, Y_Pres_N)), return_index=True, axis=0)
...
source_term = SP_vel.get_source_term(points=[X, Y], rho=rho)
# 2. Evaluate the c_N for the N conditions (see Presentation 1)
P_Neu = SP_vel.get_pressure_neumann(points=[X_Pres_N, Y_Pres_N],
                                    normals=[n_x, n_y],
                                    rho=rho,
                                    mu=mu)
...
SP_pres = Spicy([source_term], [X, Y], basis='gauss', verbose=verbose, model='scalar')
SP_pres.collocation([3, 10, 50, 200, 1000], Areas=[poly1, poly2, None, None, None],
                   r_mM=[0.03, 1], eps_l=0.87, method='clustering')
...
SP_pres.plot_RBFs(level=4) # Plot the clustering a level
...
P_calc = SP_pres.get_sol(points=[X, Y])
...
P_calc_test = SP_pres.get_sol(points=[X_p[indices_test], Y_p[indices_test]])
...
fig, axes = plt.subplots(figsize=(7, 7), nrows=3, dpi=100)
